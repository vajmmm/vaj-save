...
- 图片质量：封面使用真实本地封面解析链路，以平滑缩放填入统一竖版盒型；本地、自动下载和内嵌资源只接受可解码的竖版（含近方形）素材，横向资源不会作为封面显示；PSP 盒装图完整居中显示并保留边缘，其他平台按卡片比例裁切填充；平台 Dock 使用六个平台的官方品牌字标 SVG，统一从 4× 母版向下渲染为高对比中性色；其他标准操作图标来自 QtAwesome。
