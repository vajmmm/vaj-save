...
- 图片质量：封面使用真实本地封面解析链路，以平滑缩放填入不同平台盒型；自动下载只接受可解码的竖版（含方形）素材，横图不会写入缓存；已有本地或内置横图完整缩放并以中性留白容纳，不再裁切成竖图。平台 Dock 使用六个平台的官方品牌字标 SVG，统一从 4× 母版向下渲染为高对比中性色；其他标准操作图标来自 QtAwesome。
...
- 修复：下载缓存新增可解码及宽高校验，横图在写入前直接拒绝；旧缓存命中前再次校验，因此历史横图会自动视为未命中。画布对用户本地或存档内横图使用完整缩放和中性留白，保留原图内容；GBA 盒型也统一为竖版比例。
...
- 复核结果：自动封面不再缓存或展示横向下载图，未发现剩余 P0、P1 或 P2 方向问题。
