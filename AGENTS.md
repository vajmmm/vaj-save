...
- Keep the right panel as a fog-white detail drawer（详情抽屉） that slides over the gallery after selection. It contains the three core actions (备份 / 恢复 / 导出 ZIP), a definition list, a versions list that fills the remaining height (with 删除此版本), a note entry, ROM binding / candidate bind controls, a star, and a close action.
