...
* **The API key never reaches a log.** It travels only in the request's
  ``Authorization`` header, and :meth:`LLMCoverChooser.__repr__` omits it.
"""
...
def _extract_content(raw: bytes) -> Optional[str]:
    try:
...
class LLMCoverChooser:
    """Ask an OpenAI-compatible chat model to pick one offered file name."""
    ...
    def __repr__(self) -> str:  # pragma: no cover - trivial formatting
        return (
            f"LLMCoverChooser(model={self.model!r}, base_url={self.base_url!r}, "
            "api_key=***)"
        )
    ...
    def choose(self, candidates: Sequence[str], query: str) -> Optional[str]:
        ...
        if raw is None:
            return None
        return _match_choice(_extract_content(raw), names)
