...
高度约 74px，从左到右依次放置：菜单/品牌、搜索、排序、仅显示有更新、只看收藏、
备份有更新、设置、帮助和收藏统计。搜索占据主要弹性空间，排序和筛选保持紧凑。顶栏控件复用统一的
圆角、边框、悬停和按压状态，不引入第二种强调色。菜单含导入 ZIP 与打开备份库。

### 窄平台 Dock

Dock 宽约 116px，固定在主体左侧。平台按 `all / psp / vita / switch /
3ds / nds / gb / gbc / gba` 排列，以图标、名称和当前项浅蓝底表达筛选状态。Dock 下部保留
当前设备、刷新设备、添加设备、其他设备、FTP 拉取和本地存档入口；本地库模式另有导入 ZIP。空间不足时
...
- 单选、多选、连续选择、全选与键盘导航。
- 从设备备份、本地备份删除、恢复到用户所选目录、单个版本删除、ZIP 导出/导入和打开位置。
- ROM 候选绑定与 GB/GBC/GBA/NDS 手动绑定。
- 备注的回车/失焦保存；收藏与只看收藏。
- Checkpoint/ftpd FTP 只读增量拉取、可选记住密码及原子缓存替换。
- 文件系统卷序列号绑定存档根目录；刷新设备做全量扫描。
- 设置、版本保留规则、插入后自动备份（默认关闭）、LLM 封面消歧与连接测试。
- 恢复只拷到所选文件夹，不会写入掌机。
