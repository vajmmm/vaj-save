...
def test_app_state_accepts_a_legacy_llm_protocol_value(tmp_path: Path, monkeypatch):
    from vajsave.library import save_app_config
    ...
    save_app_config(
        {
            "llm_cover_enabled": True,
            "llm_api_key": "sk-ant",
            "llm_protocol": "anthropic",  # pre-rename value
        }
    )
    state = AppState(library_root=tmp_path / "lib")
    assert state.llm_protocol == PROTOCOL_ANTHROPIC
...
def test_set_llm_cover_completes_a_base_url_without_v1(tmp_path: Path, monkeypatch):
    from vajsave.library import load_app_config

    monkeypatch.setenv("VAJSAVE_CONFIG_PATH", str(tmp_path / "config.json"))
    state = AppState(library_root=tmp_path / "lib")
    state.set_llm_cover(base_url="https://api.deepseek.com")
    assert state.llm_base_url == "https://api.deepseek.com/v1"
    assert load_app_config()["llm_base_url"] == "https://api.deepseek.com/v1"


def test_set_llm_cover_never_duplicates_an_existing_v1(tmp_path: Path, monkeypatch):
    from vajsave.library import load_app_config
